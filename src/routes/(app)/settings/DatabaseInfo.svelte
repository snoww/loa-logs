<script lang="ts">
  import { deleteAllEncounters, deleteAllUnclearedEncounters, deleteEncountersBelowMinDuration, getDbInfo, openDbPath, optimizeDatabase } from "$lib/api";
    ...
    <button
      ...
      onclick={() => {
        dialogInfo = {
          ...
          action: async () => {
            await deleteEncountersBelowMinDuration({
    ...
    <button
      ...
      onclick={() => {
        dialogInfo = {
          ...
          action: async () => {
            await deleteAllUnclearedEncounters(settings.app.general.keepFavorites);
            refresh = !refresh;
            $open = false;
          }
        };
    ...
    <button
      ...
      onclick={() => {
        dialogInfo = {
          ...
          action: async () => {
            await deleteAllEncounters(settings.app.general.keepFavorites);
            refresh = !refresh;
            $open = false;
          }
        };
