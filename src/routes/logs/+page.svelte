        ...
        <thead class="sticky top-0 z-10 bg-zinc-900 text-xs uppercase">
          <tr>
            ...
            <th scope="col" class="hidden w-24 px-3 py-3 text-right lg:table-cell">
              <button class="hover:text-accent-500 ml-auto flex items-center" onclick={() => setSort("my_dps")}>
              ...
              <button class="hover:text-accent-500 flex items-center" onclick={() => setSort("duration")}>
                ...
                {#if $searchFilter.sort === "duration"}
                  {#if $searchFilter.order === 2}
                    <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 size-4 fill-gray-400" viewBox="0 -960 960 960">
            ...
            <th scope="col" class="w-[15%] px-3 py-3">
              <button class="hover:text-accent-500 ml-auto flex items-center" onclick={() => setSort("fight_start")}>
            ...
            <tr class="border-b border-gray-700 hover:bg-zinc-700" id="encounter-{encounter.id}">
              <td class="px-2 py-3">
                ...
                {:else}
                  <div
                    use:tooltip={{
                      content: (encounter.cleared ? "Clear " : "") + formatTimestamp(encounter.fightStart)
                    }}
                  >
              ...
              <td class="hidden truncate px-3 py-3 lg:table-cell">
                <div class="pr-1 text-right" use:tooltip={{ content: encounter.myDps.toLocaleString() }}>
          ...
          {:else}
            ...
            {:else}
              ...
              <tr class="w-screen bg-neutral-800 p-2">
                <td>
                  Meter should be turned on at character select (before entering raid at latest) for best accuracy.
                </td>
              </tr>
            {/if}
          {/each}
          ...
          <li use:tooltip={{ content: "Last" }}>
            <button class="ml-0 block px-3" onclick={() => lastPage()}>
              ...
              <svg
                ...
                viewBox="0 96 960 960"
              >
                <path d="m273.5 831.5-65.5-65 191-191-191-191 65.5-65 256 256-256 256ZM643 837V314.5h91.5V837H643Z" />
              </svg>
            </button>
          </li>
        </ul>
