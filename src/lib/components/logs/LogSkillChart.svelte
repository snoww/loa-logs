  ...
  {:else if skill && skillCast}
    <div class="px-1 pb-2">
      <div class="flex items-center pb-2 pt-1">
        ...
        >
          <svg
            class="size-7 fill-gray-400 {$focusedSkillCast.cast === 0 ? 'opacity-20' : 'hover:fill-accent-800'}"
      ...
      <div class="flex items-center space-x-2">
        <img class="rounded-xs size-7" src={$skillIcon.path + getSkillIcon(skill.icon)} alt={skill.name} />
      ...
      {:else}
        ...
        <div>
          ...
          | CDMG:
          <span class="font-semibold">{round(totalDamage !== 0 ? (modInfo.critDamage / totalDamage) * 100 : 0)}%</span>
          {#if modInfo.ba > 0}
        ...
        <div class="">
          ...
          >
          | Brand:
          <span class="font-semibold">{round(totalDamage !== 0 ? (supportBuffs.brand / totalDamage) * 100 : 0)}%</span>
          | Identity:
        ...
        <table class="mt-2 w-[60rem] table-fixed">
          <thead>
            <tr>
              ...
              <td class="w-16 font-semibold" use:tooltip={{ content: "Time since previous damage tick" }}>Ticks</td>
              <td class="w-12 font-semibold" use:tooltip={{ content: "Hit modifiers, e.g. Crit, BA, FA" }}>Mods</td>
