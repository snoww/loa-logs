<script lang="ts">
    ...
    let skill = $state<Skill>({} as Skill);
    let skillCast = $state<SkillCast>({} as SkillCast);
    let totalDamage = $state<number>(0);

    let supportBuffs = $state<SkillChartSupportDamage>({ buff: 0, brand: 0, identity: 0 });
    let modInfo = $state<SkillChartModInfo>({ crit: 0, critDamage: 0, ba: 0, fa: 0 });

    let allGroupedBuffs = $state<Map<string, Array<StatusEffectWithId>>[]>([]);
