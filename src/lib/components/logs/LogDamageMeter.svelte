  ...
  $effect.pre(() => {
    if (
      ...
      if (chartType === ChartType.AVERAGE_DPS) {
        ...
        if (Object.entries(player.skills).some(([, skill]) => skill.skillCastLog && skill.skillCastLog.length > 0)) {
          ...
          chartOptions = getSkillLogChartOld(player, $skillIcon.path, encounter.lastCombatPacket, encounter.fightStart);
        }
      } else if (chartType === ChartType.SKILL_LOG && focusedBoss) {
        let boss = enc.bosses.find((boss) => boss.name === focusedBoss);
        chartOptions = getSkillLogChartOld(boss!, $skillIcon.path, encounter.lastCombatPacket, encounter.fightStart);
      }
    }
  });
  ...
  {#if !$takingScreenshot}
    <div class="mt-2 flex justify-between" style="width: calc(100vw - 4.5rem);">
      <div class="flex divide-x divide-gray-600">
        ...
        <!--{/if}-->
        <button
          class="rounded-xs shrink-0 px-2 py-1"
        ...
        {#if encounter.cleared}
          ...
          {/if}
        {/if}
        <div class="rounded-xs relative flex items-center bg-gray-700" onfocusout={handleDropdownFocusLoss}>
