<script lang="ts">
    ...
    import { stopPropagation } from "$lib/utils/svelte";

    let filterMenu = $state(false);
    let filterTab = $state("Encounters");

    let filterDiv = $state<HTMLDivElement>();

    let { selectMode = $bindable(), refreshFn }: { selectMode: boolean; refreshFn: () => void } = $props();
    ...
    onMount(() => {
        const clickOutside = (event: MouseEvent) => {
            if (filterDiv && filterDiv.contains(event.target as Node)) return;
            if (isFilterButton(event.target as HTMLElement)) return;
            filterMenu = false;
        };
