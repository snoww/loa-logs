<script lang="ts">
    import { cardIconMap, cardIds, cardMap } from "$lib/constants/cards";
    ...
    let maxDraw = cards[0].casts;
    let drawPercentages = cards.map((skill) => (skill.casts / totalDraws) * 100);
    let relativeDrawPercentages = cards.map((skill) => (skill.casts / maxDraw) * 100);
...
<table class="relative w-full table-fixed mt-2">
            ...
            <tr class="h-6 px-2 py-1 text-3xs">
                <td class="px-1">
                    <div class="flex items-center space-x-1">
                        <img
                            class="h-5 w-5"
                            src={$skillIcon.path + getSkillIcon(card.icon)}
                            alt={card.name} />
