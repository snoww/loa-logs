    ...
    {:else}
      ...
      {#if showHeaderForOtherParties}
        ...
        {#if meterSettings.rdpsContribution}
          <th class="w-14 font-normal" use:tooltip={{ content: "% of your Damage that is from all synergies" }}>Con%</th
          >
        {/if}
        {#if meterSettings.rdpsSContribution}
          <th class="w-14 font-normal" use:tooltip={{ content: "% of your Damage that is from the Support" }}>sCon%</th>
        {/if}
        {#if meterSettings.rdpsDContribution}
          <th class="w-14 font-normal" use:tooltip={{ content: "% of your Damage that is from Dealers" }}>dCon%</th>
        {/if}
