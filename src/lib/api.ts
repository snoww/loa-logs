...
export const loadEncountersPreview = (criteria: LoadEncountersCriteria): Promise<EncountersOverview> => invoke("load_encounters_preview", { ...criteria });
...
export const getSyncCandidates = (forceResync: boolean): Promise<number[]> => invoke("get_sync_candidates", { forceResync });
...
export const onIdentityUpdate = (handler: (event: IdentityEvent) => void) => listen<IdentityEvent>("identity-update", (event) => handler({
    ...event.payload,
    timestamp: +Date.now()
}));
